...
SELECTED FILTER CONTEXT Available for SQL generation:
    system reasoning : {followup_reasoning}
...
A. TEMPORAL SCOPE: Check if time period (year,quarter,month) mentioned or not
B. METRIC DEFINITIONS: Clear business metrics exists or not
C. BUSINESS CONTEXT: Clear filtering/grouping vs unclear dimensions
D. FORMULA REQUIREMENTS: Check whether the formula exists in the metadata for the question or not.
E. METADATA MAPPING: Map the columns using the column descriptions; if not found, use the column list instead and try to map.
