...
def render_persistent_followup_questions():
    ...
    has_questions = bool(st.session_state.get('current_followup_questions', []))
    question_type = st.session_state.get('question_type_selection', 'Follow-up')
    
    print(f"🔍 render_persistent_followup_questions check: processing={is_processing}, has_questions={has_questions}, type={question_type}")
    ...
    if (hasattr(st.session_state, 'current_followup_questions') and 
        ...
        print(f"✅ Rendering {len(st.session_state.current_followup_questions)} follow-up question buttons")
        ...
        print(f"❌ NOT rendering follow-up questions (one of the conditions failed)")
                st.rerun()
...
def render_chat_message_enhanced(message, message_idx):
    ...
    # Debug logging
    is_processing = st.session_state.get('processing', False)
    print(f"🎨 Rendering message {message_idx}: type={message_type}, role={role}, historical={is_historical}, processing={is_processing}")
    ...
    if st.session_state.get('processing', False):
        if message_type in ['sql_result', 'strategic_analysis', 'drillthrough_analysis', 'followup_questions']:
            print(f"⏭️ SKIPPING message {message_idx} (type={message_type}) - currently processing")
    ...
    if is_historical and message_type in ['sql_result', 'strategic_analysis', 'drillthrough_analysis', 'followup_questions']:
        print(f"⏭️ SKIPPING message {message_idx} (type={message_type}) - marked as historical")
        return
