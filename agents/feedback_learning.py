...
VALUES (
    ...
    'SELECT * FROM (SELECT product_category, product_sub_category_lvl_2, ROUND(SUM(CASE WHEN month = 1 AND UPPER(metric_type) = UPPER("Unadjusted Scripts") THEN amount_or_count ELSE 0 END), 0) AS january_unadjusted_script_count, ROUND(SUM(CASE WHEN month = 2 AND UPPER(metric_type) = UPPER("Unadjusted Scripts") THEN amount_or_count ELSE 0 END), 0) AS february_unadjusted_script_count, ...[month 3: march_unadjusted_script_count]..., ROUND(SUM(CASE WHEN month = 1 AND UPPER(metric_type) = UPPER("Adjusted Scripts") THEN amount_or_count ELSE 0 END), 0) AS january_adjusted_script_count, ROUND(SUM(CASE WHEN month = 2 AND UPPER(metric_type) = UPPER("Adjusted Scripts") THEN amount_or_count ELSE 0 END), 0) AS february_adjusted_script_count, ...[month 3: march_adjusted_script_count]..., ROUND(SUM(CASE WHEN month = 1 AND UPPER(metric_type) = UPPER("Revenues") THEN amount_or_count ELSE 0 END) / NULLIF(SUM(CASE WHEN month = 1 AND UPPER(metric_type) = UPPER("Unadjusted Scripts") THEN amount_or_count ELSE 0 END), 0), 3) AS january_revenue_per_script, ROUND(SUM(CASE WHEN month = 2 AND UPPER(metric_type) = UPPER("Revenues") THEN amount_or_count ELSE 0 END) / NULLIF(SUM(CASE WHEN month = 2 AND UPPER(metric_type) = UPPER("Unadjusted Scripts") THEN amount_or_count ELSE 0 END), 0), 3) AS february_revenue_per_script, ...[month 3: march_revenue_per_script]..., ROUND(SUM(CASE WHEN month = 1 AND UPPER(metric_type) = UPPER("Revenues") THEN amount_or_count ELSE 0 END) / NULLIF(SUM(CASE WHEN month = 1 AND UPPER(metric_type) = UPPER("Adjusted Scripts") THEN amount_or_count ELSE 0 END), 0), 3) AS january_revenue_per_adjusted_script, ROUND(SUM(CASE WHEN month = 2 AND UPPER(metric_type) = UPPER("Revenues") THEN amount_or_count ELSE 0 END) / NULLIF(SUM(CASE WHEN month = 2 AND UPPER(metric_type) = UPPER("Adjusted Scripts") THEN amount_or_count ELSE 0 END), 0), 3) AS february_revenue_per_adjusted_script, ...[month 3: march_revenue_per_adjusted_script]... FROM prd_optumrx_orxfdmprdsa.rag.ledger_actual_vs_forecast WHERE year = 2025 AND month BETWEEN 1 AND 3 AND UPPER(ledger) = UPPER("GAAP") AND UPPER(metric_type) IN (UPPER("Unadjusted Scripts"), UPPER("Adjusted Scripts"), UPPER("Revenues")) AND UPPER(product_category) = UPPER("Specialty") GROUP BY product_category, product_sub_category_lvl_2 UNION ALL SELECT product_category, "OVERALL_TOTAL" AS product_sub_category_lvl_2, ROUND(SUM(CASE WHEN month = 1 AND UPPER(metric_type) = UPPER("Unadjusted Scripts") THEN amount_or_count ELSE 0 END), 0) AS january_unadjusted_script_count, ROUND(SUM(CASE WHEN month = 2 AND UPPER(metric_type) = UPPER("Unadjusted Scripts") THEN amount_or_count ELSE 0 END), 0) AS february_unadjusted_script_count, ...[month 3: march_unadjusted_script_count]..., ROUND(SUM(CASE WHEN month = 1 AND UPPER(metric_type) = UPPER("Adjusted Scripts") THEN amount_or_count ELSE 0 END), 0) AS january_adjusted_script_count, ROUND(SUM(CASE WHEN month = 2 AND UPPER(metric_type) = UPPER("Adjusted Scripts") THEN amount_or_count ELSE 0 END), 0) AS february_adjusted_script_count, ...[month 3: march_adjusted_script_count]..., ROUND(SUM(CASE WHEN month = 1 AND UPPER(metric_type) = UPPER("Revenues") THEN amount_or_count ELSE 0 END) / NULLIF(SUM(CASE WHEN month = 1 AND UPPER(metric_type) = UPPER("Unadjusted Scripts") THEN amount_or_count ELSE 0 END), 0), 3) AS january_revenue_per_script, ROUND(SUM(CASE WHEN month = 2 AND UPPER(metric_type) = UPPER("Revenues") THEN amount_or_count ELSE 0 END) / NULLIF(SUM(CASE WHEN month = 2 AND UPPER(metric_type) = UPPER("Unadjusted Scripts") THEN amount_or_count ELSE 0 END), 0), 3) AS february_revenue_per_script, ...[month 3: march_revenue_per_script]..., ROUND(SUM(CASE WHEN month = 1 AND UPPER(metric_type) = UPPER("Revenues") THEN amount_or_count ELSE 0 END) / NULLIF(SUM(CASE WHEN month = 1 AND UPPER(metric_type) = UPPER("Adjusted Scripts") THEN amount_or_count ELSE 0 END), 0), 3) AS january_revenue_per_adjusted_script, ROUND(SUM(CASE WHEN month = 2 AND UPPER(metric_type) = UPPER("Revenues") THEN amount_or_count ELSE 0 END) / NULLIF(SUM(CASE WHEN month = 2 AND UPPER(metric_type) = UPPER("Adjusted Scripts") THEN amount_or_count ELSE 0 END), 0), 3) AS february_revenue_per_adjusted_script, ...[month 3: march_revenue_per_adjusted_script]... FROM prd_optumrx_orxfdmprdsa.rag.ledger_actual_vs_forecast WHERE year = 2025 AND month BETWEEN 1 AND 3 AND UPPER(ledger) = UPPER("GAAP") AND UPPER(metric_type) IN (UPPER("Unadjusted Scripts"), UPPER("Adjusted Scripts"), UPPER("Revenues")) AND UPPER(product_category) = UPPER("Specialty") GROUP BY product_category) ORDER BY CASE WHEN product_sub_category_lvl_2 = ''OVERALL_TOTAL'' THEN 0 ELSE 1 END, product_sub_category_lvl_2',
    true,
    NULL,
    current_timestamp(),
    'prd_optumrx_orxfdmprdsa.rag.ledger_actual_vs_forecast'
