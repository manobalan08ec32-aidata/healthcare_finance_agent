[{"table_name":"prd_optumrx_orxfdmprdsa.rag.ledger_actual_vs_forecast","description":"This table contains ledger-level financial data including actuals, forecasts, and budget figures. It supports comparative analysis across time periods and tracks performance at the line-of-business level.","PHI_PII_Columns":[],"high_level_table":"True","useful_for":["client level revenue analysis,volume,scripts","monthly actuals analysis","high level client analysis","GAAP analysis","actuals vs forecast vs budget comparison","line-of-business level tracking","Rate analysis","Useful for analyzing revenue,expense,gross margin,other mterics,script counts,volume  at high level as long as attributes and metrics available"],"not_useful_for":["client level expense or gross margin alone","claim-level granular analysis","daily granularity"],"metrics":["revenue","expense","rate_variance","mix_variance","volume_variance","cogs","sga","ioi","membership","unadjusted_scripts","adjusted_scripts","30_day_scripts","90_day_scripts","volume","Cost %","Gross Margin %","Operating Expenses %","Operating Cost %","IOI % ","Utilization PMPM (Unadjusted)","Utilization PMPM (Adjusted)","Revenue per Script (Unadj) ","Cost per Script (Unadj) ","Revenue per Script (Unadj) ","Cost per Script (Unadj) ","Margin per Script (Unadj)","Revenue per Script (Unadj) ","Cost per Script (Unadj) ","Revenue per Script (Unadj) ","Cost per Script (Unadj) ","IOI per Script (Unadj)","Revenue per Script (Adj)","Cost per Script (Adj) ","Margin per Script (Adj)","IOI per Script (Adj)","Generic Penetration % ","Op Exp per Script (Unadjusted)","Operating Expenses % ","Operating Cost % ","Op Cost per Script (Unadjsted)","Op Exp per Script (Adjusted)","Op Exp per Script (Ajdusted)"],"attributes":["ledger","Mail service","Home Delivery","Specialty","line_of_business","transaction_date","year","month","quarter","product_category","Client_id","Client_name"],"time_grains":["monthly","quarterly","yearly"]},{"table_name":"prd_optumrx_orxfdmprdsa.rag.pbm_claims","description":"Claim-level dataset supporting financial and utilization analysis across client, pharmacy, and drug dimensions. Enables variance tracking by therapy class, manufacturer, and other attributes, with daily granularity and derived metrics like revenue per script and GDR.","PHI_PII_Columns":["member_id","member_date_of_birth"],"high_level_table":"False","useful_for":["claim-level financial analysis","client-level analysis","drug and therapy class performance","manufacturer-level insights","revenue per script and GDR metrics","line-of-business tracking","daily and monthly trend analysis","rate analysis"],"not_useful_for":["budget-level summaries","forecast comparisons","aggregated financial reporting without claim-level detail"],"metrics":["revenue","client due amount","app due amount","expense","cogs","wac","awp","unadjusted_scripts","adjusted_scripts","30_day_scripts","90_day_scripts","revenue_per_script","generic_dispensing_ratio","volume","rate_variance","mix_variance","volume_variance","gross margin"],"attributes":["claim_nbr","submit_date","client_id","client_Name","pharmacy_npi_no","drug_name","therapy_class_name","drug_manufctr_nm","line_of_business","state_cd","brand_vs_generic","client_type","gpi_no","pharmacy_name","pharmacy_type","carrier_id","member_date_of_birth","member_sex","product_category","claim_status_code"],"time_grains":["daily","monthly","quarterly","yearly"]},{"table_name":"prd_optumrx_orxfdmprdsa.rag.claim_billing","description":"This dataset helps to bring all the billing information at invoice level for client, carrier,account,group.","PHI_PII_Columns":[],"high_level_table":"False","useful_for":["claim-level billing analysis","client-level billing analysis","Invoice analysis"],"not_useful_for":["expense analysis"],"metrics":["billed amount","revenue from billing","claim_fee","claim cost","activity fee"],"attributes":["claim_nbr","claim_status_code","invoice_nbr","invoice_date","invoice_gl_date,invoice general ledger date","carrier_id","account_id","group_id","client_id","client_description","drug_name","therapy_class_name","drug_manufctr_nm","brand_vs_generic","gpi_no","billed_amount","rev_src_type","oracle_prod_code","orcl_prod_desc","actvty_category_cd","blng_entty_cd","blng_entty_name","fqa_cmpny_cd","fqa_geo_cd","fqa_aflt_cd","fqa_acnt_cd","fqa_lob_cd","fqa_dept_cd","fqa_prod_cd","fqa_sub_acnt_cd","line_of_business_name"],"time_grains":["daily","monthly","quarterly","yearly"]}]
...
- Scan for explicit attributes: "carrier", "drug", "pharmacy", "therapy", "client", "manufacturer" (case-insensitive)
- Flag as "explicit_attribute_mentioned = true/false" for later filter disambiguation
...
* **STEP 3 - FILTER DISAMBIGUATION**:
- IF explicit attribute mentioned → NO follow-up
- IF filter value in 1 column only → NO follow-up  
- IF filter value in 2+ columns AND no explicit attribute → needs_disambiguation
