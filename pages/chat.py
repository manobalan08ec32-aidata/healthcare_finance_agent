...
def render_sql_results(sql_result, rewritten_question=None, show_feedback=True):
...
def add_selection_reasoning_message(selection_reasoning):
    """
    Add selection reasoning message to chat history for proper rendering
    """
    message = {
        "role": "assistant",
        "content": selection_reasoning,
...
async def _fetch_session_history():
    ...
    Returns list of sessions with user_question, state_info, and insert_ts.
    """
    try:
        db_client = st.session_state.get('db_client')
        if not db_client:
        ...
        GROUP BY session_id ORDER BY max(insert_ts) DESC
        LIMIT 5
        """
        ...
        for i, session in enumerate(sessions):
            if isinstance(session, dict):
                print(f"  Session {i+1}: {session.get('session_id', 'NO_ID')}")
            elif isinstance(session, str):
                print(f"  Session {i+1}: {session} (string)")
            else:
...
def render_sidebar_history():
    """Render session history in the sidebar with clickable items."""
    ...
    # Display historical sessions by session_id only
    for idx, session_item in enumerate(history):
        # Handle both string and dict responses from database
        ...
        # Show just session ID (truncated and without icon for compact display)
        truncated_id = session_id[:12] + "..." if len(session_id) > 15 else session_id
        display_text = truncated_id
        
        # Create a clickable button for each session with small font
        if st.button(
            display_text, 
            key=f"history_{idx}_{session_id}",
            help=f"Click to load conversation history for session: {session_id}",
            use_container_width=True
        ):
