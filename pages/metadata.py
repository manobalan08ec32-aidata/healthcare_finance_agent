...
async def get_table_schema(db_client, table_name):
    """Get table schema using DESCRIBE command with async execution - ONLY includes columns with descriptions"""
    try:
        ...
        if results:
            ...
            for row in results:
                ...
                # Filter out junk records AND filter out columns with NULL/empty descriptions
                if (col_name not in ["# Clustering Information", "# col_name"] 
                    and description  # Only include if description is not None/empty
                    and description.strip() != ''):  # And not just whitespace
                    df_data.append({
            ...
            print(f"✅ Filtered to {len(df_data)} columns with descriptions")
            
            if df_data:  # Only create dataframe if there are rows
...
async def get_calculated_metrics(db_client, table_name):
    """Get calculated metrics from metrics_config table for the specific table - ONLY includes metrics with descriptions"""
    try:
        ...
        if results:
            ...
            for row in results:
                ...
                # Only add metrics that have non-null and non-empty descriptions
            ...
            print(f"✅ Filtered to {len(df_data)} metrics with descriptions")
...
async def display_table_metadata(table_name, display_name):
    ...
    try:
        ...
        if schema_df is not None and not schema_df.empty:
            ...
            # Already filtered in get_table_schema() to only include columns with descriptions
            schema_display = schema_df[['Column Name', 'Description']].copy()
            
            # Check if there are any rows (should always be true since filtering is done at source)
            if not schema_display.empty:
                # Create HTML table with proper text wrapping
                html_table = '<table style="width: 100%; border-collapse: collapse; margin-top: 1rem;">'
                html_table += '<thead><tr>'
                html_table += '<th style="background-color: #f8f9fa; color: #D74120; font-weight: 600; padding: 12px; text-align: left; border: 1px solid #dee2e6; width: 20%;">Column Name</th>'
                html_table += '<th style="background-color: #f8f9fa; color: #D74120; font-weight: 600; padding: 12px; text-align: left; border: 1px solid #dee2e6; width: 80%;">Description</th>'
                ...
                st.markdown('<p style="color: #999; font-size: 0.9rem; margin: 1rem 0 0 0;">No columns with descriptions available.</p>', unsafe_allow_html=True)
            ...
            if calculated_metrics_df is not None and not calculated_metrics_df.empty:
                ...
                html_calc_table += '<th style="background-color: #f8f9fa; color: #D74120; font-weight: 600; padding: 12px; text-align: left; border: 1px solid #dee2e6; width: 25%;">Column Name</th>'
                html_calc_table += '<th style="background-color: #f8f9fa; color: #D74120; font-weight: 600; padding: 12px; text-align: left; border: 1px solid #dee2e6; width: 75%;">Description</th>'
