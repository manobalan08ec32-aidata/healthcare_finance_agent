**REASONING:**
- Extracted: metrics=[membership], attributes=[NONE] (summary query), filters=[MPDOVA → carrier_id column]
- Requirements: Need membership metric + carrier_id column for filtering
- Eliminated: pbm_claims (missing membership metric), claim_billing (missing membership metric)
- Selected: ledger_actual_vs_forecast - has membership metric and can handle summary queries
...
<json>
{
  ...
  "selection_reasoning": "Only table containing membership metric for summary analysis. Other tables lack membership data.",
  "high_level_table_selected": true,
  "selected_filter_context": "columns: [carrier_id], values: [MPDOVA]"
}
</json>
 ...
 [
  {
    ...
    "metrics": ["revenue", "client due amount","Line count", "app due amount", "expense", "cogs", "Gross Margin %", "wac", "awp", "unadjusted_scripts", "adjusted_scripts", "30_day_scripts", "90_day_scripts", "revenue_per_script", "generic_dispensing_ratio", "volume", "rate_variance", "mix_variance", "volume_variance", "gross margin"],
  ...
  },
  {
    ...
    "metrics": ["revenue", "expense", "rate_variance", "mix_variance", "revenue per script","volume_variance", "cogs", "sga", "ioi", "membership", "unadjusted_scripts", "adjusted_scripts","Line count" ,"30_day_scripts", "90_day_scripts", "volume", "Cost %", "Gross Margin %", "Operating Expenses %", "Operating Cost %", "IOI %", "Utilization PMPM (Unadjusted)", "Utilization PMPM (Adjusted)", "Revenue per Script (Unadj)", "Cost per Script (Unadj)", "Margin per Script (Unadj)", "IOI per Script (Unadj)", "Revenue per Script (Adj)", "Cost per Script (Adj)", "Margin per Script (Adj)", "IOI per Script (Adj)", "Generic Penetration %", "Op Exp per Script (Unadjusted)", "Op Cost per Script (Unadjusted)", "Op Exp per Script (Adjusted)"],
  ...
  {
    "table": "prd_optumrx_orxfdmprdsa.rag.claim_billing",
    "name": "CBS Billing",
