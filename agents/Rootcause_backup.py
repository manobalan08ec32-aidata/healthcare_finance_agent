...
class DatabricksClient:
    ...
    def __init__(self):
        ...
        token_payload = {
            ...
            "scope": f"{self.DATABRICKS_HOST}/.default"
        }
    ...
    async def sp_vector_search_columns(
        ...
        Args:
            ...
            tables_list: List of table names (works with 1 or more tables)
            num_results_per_table: Results to return per table
            index_name: Vector search index name
        
        Returns:
            List of dicts with table_name, col_embedding_content, llm_context
        """
        ...
        # Query each table separately to guarantee num_results_per_table
        for table_name in tables_list:
            ...
            try:
                results = index.similarity_search(
                    ...
                    # ✅ Filter to single table (guaranteed results)
                ...
                print(f" ✅ {table_name}: {len(results.get('result', {}).get('data_array', []))} columns")
            
            except Exception as e:
                print(f" ⚠️ {table_name}: Error - {e}")
                continue
        
        print(f"✅ Total: {len(all_results)} columns from {len(tables_list)} table(s)")
